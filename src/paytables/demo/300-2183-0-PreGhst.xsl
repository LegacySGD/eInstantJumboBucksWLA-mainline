				...
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
 						r.push('<tr>');
						r.push('<td class="tablehead" colspan="' + winningNums.length + '">');
						...
						{
							r.push('<td class="tablebody">' + (winningNums[i]).toString() + '</td>');
						}
					...
					// Input: "35,9,17,24,29,12|16:G,22:D,28:B,8:C,18:F,37:D,33:I,2:E,11:F,17:A,36:J,6:K,19:D,14:K,27:E,40:B,4:J,31:H,13:I,20:H"
					// Output: ["35", "9", "17", "24", "29", "12"]
					...
					function checkMatch(winningNums, boardNum)
					{
						for (var i=0; i<winningNums.length; i++)
						{
							if (winningNums[i] == boardNum || (boardNum >= 'T' && boardNum <= 'Z'))
							{
								return true;
							}
						}
					...
					// Input: symbol (e.g. 'T' to 'Z'), or number (e.g. '1' to '40')
					// Output: translated text or number.
					function translateOutcomeNumber(outcomeNum, translations)
					{
						if (outcomeNum >= 'T' && outcomeNum <= 'Y')
						{
							return getTranslationByName('multiplier'+outcomeNum, translations);
						}
